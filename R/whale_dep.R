...
whale_dep <- function(channel, station) {
  ...
  dat <- get_data(channel = channel, sta_num = station)
  depth = dat[[1]]
  cat = dat[[2]]

  plot_dat <- make_plot_data(depth = depth, cat = cat, strat = strat, names = names)
  ...
  plt <- plot_data(depth_strat = depth_strat, spp_sum_strat = spp_sum_strat, dep = dep, catch = catch)
}
